...
/**
 * 
 * @TODO: WARNING: THIS IS A COPY FROM core-task-impl to provide session loading capability!!!!
 * 
...
public class OpenSessionTask extends AbstractOpenSessionTask {
	...
	public final class OpenSessionWithoutWarningTask extends AbstractTask {
			...
			try {
				try {
					...
					reader = serviceRegistrar.getService(CySessionReaderManager.class)
							.getReader(sessionFile.toURI(), sessionFile.getName());
					
					if (reader == null)
						throw new NullPointerException("Failed to find appropriate reader for file: " + sessionFile);
		...
		private void changeCurrentSession(CySessionManager sessionManager, TaskMonitor tm) throws Exception {
			...
			if (newSession == null)
				throw new NullPointerException("Session could not be read for file: " + sessionFile);

			sessionManager.setCurrentSession(newSession, sessionFile.getAbsolutePath());
			
			tm.setProgress(1.0);
			tm.setStatusMessage("Session file " + sessionFile + " successfully loaded.");
			
			// Add this session file URL as the most recent file.
			serviceRegistrar.getService(RecentlyOpenedTracker.class).add(sessionFile.toURI().toURL());
		}
	}
